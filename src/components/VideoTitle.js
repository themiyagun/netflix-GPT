...
const VideoTitle = ({overview,title}) => {
  return (
    <div className='pt-[20%] px-12 absolute w-full aspect-video text-white bg-gradient-to-r from-black'>
      ...
      <h5 className='hidden md:block py-6 w-1/4'>{overview}</h5>

      <div className='flex'>
        <button className='p-2 bg-gray-500 text-white px-10 text-lg bg-opacity-50 rounded-lg'>▶️ Play</button>
        <button className='hidden md:block mx-2 p-2 bg-gray-500 text-white px-10 text-lg bg-opacity-50 rounded-lg'>More Info</button>
      </div>
    </div>
  )
}
